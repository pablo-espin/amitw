...
One-way cable connection animation → sparks → `PowerOn()`: notifies StatsSystem + dialogue (`electricity_solved`), then either the normal sequential light-up (area lights, lightbulb material swaps, server-rack activation via `ServerRackMaterialController.SetState`, clue reveal, lightbulb registration with LockdownManager) or, if lockdown is active, `PostLockdownPowerSequence()` (3 s of light, clue revealed, then lockdown re-asserts). Subscribes to `OnLockdownPhaseChanged` to kill its area lights. Code default `KWH-365`. API: `InteractWithCable()`, `IsCableConnected()`, `ServersToActivate` (public getter for its `serversToActivate` rack array — lets `ServerEmergencyDriftController` exclude these racks from its random drift).
...
### ServerRackMaterialController.cs — per-rack material state machine `ServerState { Normal, Emergency, PoweredOff, HighActivity }` with optional fade transitions. `SetState(state, delay)`, `GetCurrentState()`; statics: `SetAllRacksEmergencyMode(bool, cascade, speed)` (used by LockdownManager), `PowerOnServersInArea(center, radius)`, `SetHighActivityZone(center, radius)`, `GetAllControllers()` (snapshot list, used by ServerEmergencyDriftController), `DebugLogAllStates()`.
### ServerEmergencyDriftController.cs — subscribes to `StatsSystem.OnMemoryHealthUpdated`; as memory health falls, randomly promotes a capped fraction (`maxEmergencyFraction`, default 0.35) of running (`Normal`-state) racks to `Emergency`, each after a random 1.5-4 s delay, so the server room degrades gradually pre-lockdown. Builds its eligible pool one frame after `Start()` (after other racks/ElectricityClueSystem set their initial state), excluding every rack in `ElectricityClueSystem.ServersToActivate`. Stops promoting on `LockdownManager.OnLockdownInitiated` (start of the escape window) so the racks stay frozen through the escape window and don't dilute the full-cascade `SetAllRacksEmergencyMode` that fires later at `StartFinalLockdown`.
